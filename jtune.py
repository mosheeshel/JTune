...
def _generate_rotated_gc_log_name(gc_log_file, log_number):
    return gc_log_file + "." + str(log_number)

def _get_rotated_log_file(gc_log_file):
    """Function will scan existing log files to determine latest rotated log, if none found will return
       non rotated file name.
    """
    log_number = 0
    while os.path.isfile(_generate_rotated_gc_log_name(gc_log_file, log_number)):
        log_number += 1
    ...
    return _generate_rotated_gc_log_name(gc_log_file, (log_number - 1))

def get_gc_log_file(procdetails=None):
    ...
    if procdetails['gc_file_rotation']:
        return _get_rotated_log_file(gc_log_file)
    else:
        return gc_log_file
